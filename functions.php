...
function bootstrapwp_theme_setup() {
  ...
  register_nav_menus( array(
    ...
    'career-center-menu' => __( 'Career Center Menu', 'bootstrapwp' ),
    'blog-menu' => __( 'Blog Menu', 'bootstrapwp' ),
  ) );
...
################################################################################
  function bootstrapwp_js_loader() {
       wp_enqueue_script('bootstrapjs', get_template_directory_uri().'/js/bootstrap.min.js', array('jquery'),'2.1.2', true );
       ...
       wp_enqueue_style('bootstrapcss',  get_template_directory_uri().'/css/styles/bootstrapwp.css', array(), '2.1');
       wp_enqueue_style('c-staff-member',  get_template_directory_uri().'/css/styles/c-staff-member.css', array(), '1.1');
       wp_enqueue_style('c-main-menu',  get_template_directory_uri().'/css/styles/c-main-menu.css', array(), '1.0');
       ...
       wp_enqueue_style('utils-flex-master',  get_template_directory_uri().'/css/utils-flex-master/index.css', array(), '1.0');       
       wp_enqueue_style('utility-typography',  get_template_directory_uri().'/css/styles/utility-typography.css', array(), '1.0');
       wp_enqueue_style('overrides',  get_template_directory_uri().'/css/styles/overrides.css', array(), '1.0');


      // Make Placeholders work in IE 8 and 9
      wp_enqueue_script('placeholderjs', get_template_directory_uri().'/js/placeholder.min.js', array('jquery'),'1.0', true );

      // Custom Scripts for Recruitment Partners
      wp_enqueue_script('scripts', get_template_directory_uri().'/js/scripts.js', array('jquery'),'1.0', true );

      // Only load if viewed on desktop
      include 'includes/mobile-detect.php';
      $detect = new Mobile_Detect();

      if(!($detect->isMobile())) {
         wp_enqueue_script('desktop-scripts', get_template_directory_uri().'/js/desktop-scripts.js', array('jquery'),'1.0', true );
      }

      if ( is_page_template('page-contact.php')) {
         wp_enqueue_style('font-awesome','https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css');
         wp_enqueue_script('acf-google-map', get_template_directory_uri().'/js/acf-google-map.js', array('jquery'),null, true );
      }
...
function my_acf_google_map_api( $api ){
	$api['key'] = 'AIzaSyD8GIPzD4aXMqbGuQwlk5sddgjYhiF7Jos';
	return $api;
}
...
function bootstrapwp_posted_on() {
	printf( __( '<span class="byline"><span class="sep">Written by </span><span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span><span class="sep"> on </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a>', 'bootstrap' ),
...
function bootstrapwp_autoset_featured_img() {
  ...
  if ($post_thumbnail == true ){
    return the_post_thumbnail($id, 'large');
  }
...
require( get_template_directory() . '/includes/extend-functions.php' );
